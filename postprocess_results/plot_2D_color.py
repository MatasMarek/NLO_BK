...
def plot_2d_color(y_ind, calculation):
    ...
    # plot the color map
    plt.imshow(N, interpolation='nearest', origin='lower', aspect='auto', vmin = 0., vmax=0.8, extent=[grid_in_b[0], grid_in_b[-1], grid_in_r[0], grid_in_r[-1]])
    plt.colorbar()
    # title = str(calculation['dimensionality_of_N']) + 'D BK equation at ' + str(calculation['order_of_BK']) + ' order Y=' + str(y)
    title = calculation['run_name'] + ' Y=' + str(y)
    plt.title(title)
